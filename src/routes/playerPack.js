...
playerPackRouter.get('/', async (req, res) => {
  ...
  /* ⭐⭐⭐ 29al — COVERAGE BACKSTOP: EVERY NFL TEAM FIELDS A TIGHT END, SO EVERY NFL TEAM MUST HAVE ONE HERE.
     ...
     The inclusion gate above is right about what it is for. It exists to keep long-retired players off the
     board — an Adrian Peterson with a stale harvested ADP and no projection — and the test it uses is "does
     anyone currently project or price this man". That test has a blind spot it cannot see from the inside: a
     CURRENT player whose projection has not landed yet and whose ADP has not been published looks exactly
     like a retired one. In August, with feeds still filling in, that is an ordinary state for a starting
     tight end on a team the projection provider has not got to.
     The cost is not symmetric, either. A retired player on the board is a row you scroll past. A missing
     player is a pick you cannot record — somebody in the league drafts him and the draft board simply cannot
     represent what happened, which breaks every number downstream of it.
     ⚠ THE RULE IS ABOUT TEAM-POSITION COVERAGE, NOT ABOUT INDIVIDUALS. "Let anyone in who has an ADP" would
       re-open the retired-player hole this gate closed. But a real NFL team with ZERO players at a position
       the league drafts is not a judgement call — it is a data gap, provably, because the position exists on
       every roster in the league. So the only players readmitted are the ones filling a hole that cannot
       legitimately be empty, and they arrive with no projection, which the client already renders as "—" and
       already keeps out of its recommendations.
     ⚠ CAPPED, AND ONLY FOR POSITIONS THIS LEAGUE DRAFTS. Three per gap is enough for a real depth chart and
       small enough that a wholesale feed outage cannot flood the board through this door. */
